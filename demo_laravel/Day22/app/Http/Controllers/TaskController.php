...
class TaskController extends Controller
{
    ...
    if (!auth()->user()->can('update', $task)) {
        return redirect()->route('tasks.index')->with('error', 'Bạn không đủ quyền để chỉnh sửa task.');
    }

    \Log::info('Editing Task: ', ['task' => $task]);
    ...
    {
    if (!auth()->user()->can('update', $task)) {
        return redirect()->route('tasks.index')->with('error', 'Bạn không đủ quyền để cập nhật task.');
    }
    ...
    return redirect()->route('tasks.index')->with('success', 'Task cập nhật thành công.');
    }
